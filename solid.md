...
| Principle | Description                                                              | Risk If Not                                      | Category    | Basic Principles |
| :-------: | ------------------------------------------------------------------------ | ------------------------------------------------ | ----------- | ---------------- |
| S         | 一个类中不要混跟本类无关的代码                                           | 无关部分的变动会引起该类改动                     | Class       | 封装变化         |
| O         | 类写好测试好并且被其他类调用时，尽量不要修改类，如有必要，进行拓展(继承) | 类一旦修改, 依赖于该类的其他类可能就无法正常运行 | Class       | 封装变化         |
| L         | 子类要兼容父类, 涉及到成员变量，方法参数，方法返回值和方法内异常         | 在要求传入父类的地方传入了子类可能会出错         | Inheritance | 组合优于继承     |
| I         | 接口方法不要全写在一个接口内，要适当细分                                 | 一些类要强制实现一些自己并不需要的接口类方法     | Interface   | 封装变化         |
| D         | 高层次不要直接依赖低层次代码, 中间设置接口                               | 低层次类中的改动可能会引起高层次类的变动         | Interface   | 面向接口编程     |
...
- 子类方法的`参数类型必须`与其超类的参数类型`相匹配`或`更加抽象`。
  - 子类不应该加强其前置条件。比如子类要求传入`正数`，而父类要求传入`int`。
  - 子类不能削弱其后置条件。比如父类有方法在使用后会**关闭**所有活跃的数据库连接，子类中却**保持**了活跃的数据库连接。
- 子类方法的`返回值类型`必须与超类方法的`返回值类型`或是其`返回值子类别`相匹配。
- 子类方法中的`异常`必须与超类方法中`异常`匹配或者超类方法中`异常子类别`相匹配。
- 超类的不变量必须保留。超类中的常量或者固定成员变量继承到子类时，不要直接修改，而要保留，有需要再拓展。
- 子类不能修改超类中私有成员变量的值。
