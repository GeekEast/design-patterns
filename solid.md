...
| :-------: | ------------------------------------------------------------------------ | ------------------------------------------------ |
| S         | 一个类中不要混跟本类无关的代码                                           | 无关部分的变动会引起该类改动                     |
| O         | 类写好测试好并且被其他类调用时，尽量不要修改类，如有必要，进行拓展(继承) | 类一旦修改, 依赖于该类的其他类可能就无法正常运行 |
| L         | 子类要兼容父类, 涉及到成员变量，方法参数，方法返回值和方法内异常         | 在要求传入父类的地方传入了子类可能会出错         |
| I         | 接口方法不要全写在一个接口内，要适当细分                                 | 一些类要强制实现一些自己并不需要的接口类方法     |
| D         | 高层次不要直接依赖低层次代码, 中间设置接口                               | 低层次类中的改动可能会引起高层次类的变动         |
