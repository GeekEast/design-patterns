...
你可将程序的变化部分放入独立的模块中，保护其他代码不受负面影响。 最终，你只需花**较少时间**就能让程序恢复正常工作， 或是实现并测试修改的内容。 **你在修改 程序上所花的时间越少，就会有更多时间来实现功能**。
...
聚合与组合类似，但是聚合是更松弛的组合的变体，一个对象可引用另外一个对象，但并不管理其生命周期。例如: 一辆汽车上有司机，但是司机也可能会使用另外一辆汽车，或者选择步行而不使用汽车。